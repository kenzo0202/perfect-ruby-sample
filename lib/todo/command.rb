...
module Todo
  ...
  class Command
    ...
    def display_format(id,name,content,status)
      name_size = 20 - full_width_count(name)
      content_size = 40 - full_width_count(content)

      [id.to_s.rjust(4),name.ljust(name_size),content.ljust(content_size),status.center(10)].join(' | ')
    end
